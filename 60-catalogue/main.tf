...
resource "aws_launch_template" "catalogue" {
    ...
    tag_specifications {
        ...
        # EC2 tags created by Auto scaling group
        tags = merge(
            local.common_tags,
            {
                Name = "${var.project}-${var.environment}-catalogue"
    ...
    # volume tags created by Auto scaling group
    tag_specifications {
...
resource "aws_autoscaling_group" "catalogue" {
    ...
    ### this dynamic "tag" will repeat for every iteration
    dynamic "tag" {
        for_each = merge(
            local.common_tags,
            {
    ...
    instance_refresh {
        ...
        triggers = ["launch_template"] ### it will trigger once launch template changes
    }
...
resource "aws_autoscaling_policy" "catalogue" {
    ...
    policy_type = "TargetTrackingScaling"
    ### cooldown = 120  ## this may not required in real case, if required do uncomment
