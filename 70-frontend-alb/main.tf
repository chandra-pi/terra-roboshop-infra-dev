module "frontend_alb" {
    source = "terraform-aws-modules/alb/aws"   ##### terraform public module from git
    version = "9.16.0"
    internal = false
...
resource "aws_lb_listener" "frontend_alb" {
    ...
    port = "443"
    protocol = "HTTPS"
    ssl_policy = "ELBSecurityPolicy-2016-08"  ### this will make issue in CDN, recommended "ELBSecurityPolicy-TLS13-1-2-Res-2021-06"
...
resource "aws_route53_record" "frontend_alb" {
    zone_id = var.zone_id
    name = "${var.environment}.${var.zone_name}" #dev.devaws84s.online
    type = "A"
